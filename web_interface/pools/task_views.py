...
from web_interface.models import AWSAccessKey, VPC, CondorPool, CondorJob, Task, Subtask
from web_interface import models, aws
from django.utils.decorators import method_decorator
from django.contrib.auth.decorators import login_required, permission_required
import sys
from web_interface.account.account_viewsN import MyAccountView
from django.contrib.auth.forms import PasswordChangeForm
from boto.vpc import VPCConnection
from boto.ec2 import EC2Connection
from web_interface.aws import vpc_tools, ec2_tools
#import task_tools, condor_tools
from web_interface.pools import task_tools, condor_tools
...
from web_interface import task_plugins
from web_interface.task_plugins import base, tools, plugins
...
class NewTaskView(RestrictedFormView):
    ...
    def form_valid(self, form,  *args, **kwargs):
        ...
        if not os.path.exists(user_dir_path):
            ...
            slog.debug("user directory path" + str(user_dir_path))
        ...
        for field_name, field_object in extra_fields:
            ...
            if isinstance(field_object, forms.FileField) and isinstance(form.cleaned_data[field_name], UploadedFile):
                try:
                    ...
                    slog.exception(e)
                    serror_messages = ['An error occured while preparing the task data files',
                                       str(e),]
        ...
        try:
            if not directory_created:
                ...
                os.mkdir(task_dir_path)
                #added by HB
                slog.debug('******** directory now created:' + str(task_dir_path))
...
class TaskDetailsView(RestrictedView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if task.status == 'error':
            ...
            slog.debug("----------> error_message: ")
            slog.debug(task.get_custom_field('error'))
            slog.exception("******** more details: ")
