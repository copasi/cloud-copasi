...
do_install() {
	...
	if [ -f "/etc/condor/condor_config" ]; then
		...
		ubuntu|debian)
			...
			EOF
			;;
		scientific|centos|rhel|rocky|amzn)
			PACKAGE="condor"
			if [[ ! $ROLE ]]; then
				PACKAGE="minicondor"
			fi
	...
	case $lsb_dist in
		ubuntu|debian)
			(
				...
				$sh_c "curl -fsSL ${DOWNLOAD_URL}/repo/keys/HTCondor-${CHANNEL_DIR}-Key | apt-key add -"

				# Our repository redirects to https even if you try to
				# avoid it, and Debian 9 doesn't come with this by default.
				$sh_c "apt-get install apt-transport-https"
		...
		scientific|centos|rhel|rocky|amzn)
			...
			# The Amazon Linux container doesn't include ps by default.
			if [ "${lsb_dist}" = "amzn" ]; then
				echo -e "\n# Installing ps"a
				$sh_c "yum install -y procps"
			fi
			...
			#
			case ${lsb_dist}.${dist_version} in
				rocky.8 ) $sh_c "dnf -y install dnf-plugins-core"
